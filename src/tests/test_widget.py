...
def test_default_list():
    ...
    #简单验证下列表内容
    assert data['list'][0]['title'].decode('utf-8') == u'报销单描述'
    assert data['list'][9]['title'].decode('utf-8') == u'请上传发票照片'
...
def test_create_input_widget():
    ...
    assert rs['status'] == 0
    assert rs['data']['title'].decode('utf-8') == u'文本控件'
...
def test_create_money_widget():
    ...
    rs = global_params.post('/widget/create', param)
    assert rs['status'] == 0
    assert rs['data']['title'].decode('utf-8') == u'金额控件'
...
def test_update_widget():
    ...
    rs = global_params.post('/widget/create', param)
    assert rs['status'] == 0
    assert rs['data']['title'].decode('utf-8') == u'新的控件标题'
    assert rs['data']['remark'].decode('utf-8') == u'新的控件描述'
